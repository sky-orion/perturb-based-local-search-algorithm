            ...
            for arclena in allarclena:
                for j in range(i + 1, len(route)):  # 两条路径不一样
                    ...
                    for arclenb in allarclenb:
                        for a in range(1, len(subtouri.route) - arclena - 1):
                            for b in range(1, len(subtourj.route) - arclenb - 1):
                                ci, cj = a, a + arclena - 1
                                cs, cr = b, b + arclenb - 1
                                gain = self.computergain(subtouri, subtourj, ci, cj, cs, cr)
                                # print(gain)
...
class random_perturbation():
    ...
    def operate(self, route):
        ...
        for i in range(H):
            r = np.random.rand()
            if r <= 0.5:
                route = self.twoExchangemove_operators.operate(route=route, random_choice=True)
            else:
                route = self.twoOptmove_operators.operate(route=route, random_choice=True)
        return route
...
if __name__ == '__main__':
    ...
    for i in range(testrange):
        route = operator.operate(route, random_choice=random_choice)
    cost = calculate_cost(route, printroute=printroute)
    ...
    route = perturbation.operate(route=route)
    cost = calculate_cost(route, printroute=printroute)
